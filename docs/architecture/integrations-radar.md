...
Streaming status (Open WebUI SSE): `正在拉取 Radar…` at FA check-in start;
`正在分析 FA 背景…` while the LLM reads title/description/diagnosis;
`正在下载附件 (n/m)…` during on-demand / related-evidence pulls (including
check-in strong-related materialization);
`正在提取 Fail items…` while the LLM extracts structured fails from the corpus;
`正在生成 AI Summary…` while the narrative check-in summary is written;
