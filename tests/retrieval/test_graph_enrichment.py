...
def test_build_graph_enrichment_from_tokens(tmp_path: Path) -> None:
    ...
    assert "U101" in text.upper() or "component:iphone/logan/p1:U101" in text
...
def _power_graph() -> KnowledgeGraph:
    ...
    g = KnowledgeGraph()
    rail = rail_node_id("iphone", "logan", "p1", "VBAT")
    reg = component_node_id("iphone", "logan", "p1", "U5")
    load = component_node_id("iphone", "logan", "p1", "U0902")
    g.add_node(GraphNode(
        id=rail, type=NODE_RAIL, product="iphone", project="logan", build="p1",
        attributes={"name": "VBAT", "role": "output"},
    ))
    g.add_node(GraphNode(
        id=reg, type=NODE_COMPONENT, product="iphone", project="logan", build="p1",
        attributes={"name": "U5"},
    ))
    g.add_node(GraphNode(
        id=load, type=NODE_COMPONENT, product="iphone", project="logan", build="p1",
...
def test_resolve_power_seed_folds_separators(tmp_path: Path) -> None:
    ...
    expected = rail_node_id("iphone", "logan", "p1", "VBAT")
    # Separator variants must all resolve to the same rail node.
    assert (
        _resolve_power_seed(pw, ["V_BAT"], product="iphone", project="logan", build="p1")
        == expected
    )
    assert (
        _resolve_power_seed(pw, ["VBAT"], product="iphone", project="logan", build="p1")
        == expected
    )
    assert (
        _resolve_power_seed(pw, ["NOPE"], product="iphone", project="logan", build="p1")
        is None
    )
...
def test_power_tree_empty_falls_back_to_neighborhood(tmp_path: Path) -> None:
    ...
    g = KnowledgeGraph()
    vcc3 = rail_node_id("iphone", "logan", "p1", "VCC3")
    g.add_node(GraphNode(id=vcc3, type=NODE_RAIL, product="iphone", project="logan", build="p1",
                         attributes={"name": "VCC3", "role": "output"}))
    u7 = component_node_id("iphone", "logan", "p1", "U7")
    g.add_node(GraphNode(id=u7, type=NODE_COMPONENT, product="iphone", project="logan", build="p1",
                         attributes={"name": "U7"}))
    ...
    assert "U7" in text.upper() or "component:iphone/logan/p1:U7" in text
...
def test_power_query_without_seed_falls_back_to_none(tmp_path: Path) -> None:
    ...
    g = KnowledgeGraph()
    g.add_node(GraphNode(
        id=component_node_id("iphone", "logan", "p1", "U101"),
        type=NODE_COMPONENT, product="iphone", project="logan", build="p1",
