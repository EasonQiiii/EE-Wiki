...
def _chunk(chunk_id: str, project: str, build: str) -> HybridChunk:
    return HybridChunk(
        chunk_id=chunk_id,
        content=f"content for {chunk_id}",
        metadata={"project": project, "build": build, "document_type": "engineering_note"},
        citation={"source_file": f"data/raw/{project}/{build}/note/{chunk_id}.md"},
    )


def test_scope_tier_derivation(data_layout) -> None:
    assert scope_tier("logan", "p1", data_layout) == SCOPE_TIER_BUILD
    assert scope_tier("logan", "common", data_layout) == SCOPE_TIER_PROJECT_COMMON
    assert scope_tier("global", "global", data_layout) == SCOPE_TIER_GLOBAL


def test_build_cascade_phases_from_ranks_groups_inherit_revisions(data_layout) -> None:
    scope_ranks = {
        ("logan", "p1"): 0,
        ("logan", "p2"): 0,
        ("logan", "common"): 1,
    ...
    assert phases[0].scope_pairs == frozenset({("logan", "p1"), ("logan", "p2")})
    assert phases[1].tier == SCOPE_TIER_PROJECT_COMMON
    assert phases[2].tier == SCOPE_TIER_GLOBAL
...
def test_assemble_mixed_quota_build_primary_with_supplement() -> None:
    reranked = {
        SCOPE_TIER_BUILD: [
            (0.9, _chunk("b1", "logan", "p1")),
            (0.8, _chunk("b2", "logan", "p1")),
        ],
        SCOPE_TIER_PROJECT_COMMON: [(0.7, _chunk("c1", "logan", "common"))],
        SCOPE_TIER_GLOBAL: [(0.95, _chunk("g1", "global", "global"))],
    }
