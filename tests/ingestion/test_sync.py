...
def test_log_skipped_raw_files_warns_on_key_and_numbers(
    ...
    warnings = log_skipped_raw_files(
        sync_config.raw_dir,
        sync_config.data_layout,
        iwork_enabled=False,
    )

    log_warnings = [record.message for record in caplog.records if record.levelname == "WARNING"]
    assert any("slides.key" in message and ".key" in message for message in log_warnings)
    assert any("sheet.numbers" in message and ".numbers" in message for message in log_warnings)
    assert any("archive.zip" in message for message in log_warnings)
    assert any("slides.key" in warning.raw_path.name for warning in warnings)
    assert any("archive.zip" in warning.raw_path.name for warning in warnings)
